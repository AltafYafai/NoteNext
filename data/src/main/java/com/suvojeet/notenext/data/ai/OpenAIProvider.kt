...
@Singleton
class OpenAIProvider @Inject constructor() : AIProviderService {
    ...
    private suspend fun <T> executeWithRetry(
        ...
        for (model in models) {
            ...
            while (currentRetry <= 2) {
                try {
                    // Note: This needs Retrofit setup with OpenAI base URL
                    // For now, returning placeholder - will be connected in NetworkModule
                    throw Exception("OpenAI integration pending Retrofit setup")
                } catch (e: Exception) {
                    lastException = e
